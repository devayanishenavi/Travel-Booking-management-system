UPDATE Employee_DTravels SET Emp_Position = 'Driver' WHERE EmployeeID = 2;
SELECT * FROM Employee_DTravels;

CREATE TABLE CustomerTravelDoc AS SELECT CustomerID, FullName, Identity_proof FROM CustomerDetails_DTravels;
SELECT * FROM CustomerTravelDoc;

ALTER TABLE CustomerTravelDoc RENAME TO Customer_documents;
SELECT * FROM Customer_documents;
 
ALTER TABLE CustomerDetails_DTravels
